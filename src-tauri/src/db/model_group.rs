...
use super::{label::{self, Label}, model::Model};
...
pub async fn set_group_id_on_models(group_id : Option<i64>, model_ids : Vec<i64>, db : &super::db::Db)
{
    let in_query = model_ids.iter().map(ToString::to_string).collect::<Vec<String>>().join(",");
    let formatted_query = format!(
        "UPDATE models
	     SET model_group_id = ?
	     WHERE model_id IN ({})", in_query);
...
pub async fn add_empty_group(group_name : &str, db : &super::db::Db) -> i64
{
    let result = sqlx::query!(
        "INSERT INTO models_group (group_name) VALUES (?)", group_name)
        .execute(db)
        .await;
...
{
    sqlx::query!(
        "UPDATE models_group SET group_name = ? WHERE group_id = ?", group_name, group_id)
        .execute(db)
        .await
...
{
    sqlx::query!(
        "DELETE FROM models_group WHERE group_id = ?", group_id)
        .execute(db)
        .await
