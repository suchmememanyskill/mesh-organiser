...
impl ImportStateEmitter for TauriImportStateEmitter {
    fn status_event(&self, status: &ImportState) {
        let _ = self
            .handle
            .emit(IMPORT_STATUS_EVENT, status.status.to_string());
    }

    fn model_total_event(&self, status: &ImportState) {
        let _ = self
            .handle
            .emit(IMPORT_MODEL_TOTAL_EVENT, status.model_count);
    }
    ...
    fn thumbnail_count_event(&self, status: &ImportState) {
        let _ = self.handle.emit(
            IMPORT_THUMBNAIL_COUNT_EVENT,
            status.finished_thumbnails_count,
        );
    }
...
pub fn import_state_new_tauri(
    ...
    ImportState::new_with_emitter(
        origin_url,
        recursive,
        delete_after_import,
        import_as_path,
        app_state.get_current_user(),
        Box::new(TauriImportStateEmitter {
