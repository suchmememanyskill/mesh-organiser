...
ARG TARGETPLATFORM
ARG BUILDPLATFORM
RUN apt update && apt install -y wget xz-utils nodejs libfontconfig1-dev libssl-dev openssl build-essential cmake
RUN wget -qO- https://get.pnpm.io/install.sh | ENV="$HOME/.shrc" SHELL="$(which sh)" sh -
RUN chmod +x ./build-web-musl.sh && ./build-web.sh
